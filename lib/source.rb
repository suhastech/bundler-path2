...
class Source < Bundler.plugin("1", :source)
  ...
  def initialize(options)
    ...
    if options["path2"]
      @path = Pathname.new(options["path2"])
...
private
  ...
  def has_app_cache?
    Bundler::SharedHelpers.in_bundle? && app_cache_path.exist?
  end
  ...
  def generate_bin(spec, disable_extensions = false)
    ...
    Bundler::SharedHelpers.chdir(gem_dir) do
      installer = Path2::Installer.new(spec, :env_shebang => false)
